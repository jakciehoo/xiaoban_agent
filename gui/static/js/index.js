new Vue({
    ...
    methods: {
        ...
        showContent(targetId) {
            ...
            // 遍历所有内容区域元素，显示或隐藏根据目标ID判断
            for (var i = 0; i < contentDivs.length; i++) {
                if (contentDivs[i].getAttribute("id") === targetId) {
                    contentDivs[i].style.display = ""; // 显示该内容区域
                } else {
                    contentDivs[i].style.display = "none"; // 隐藏除目标外的其他内容区域
                }
            }
                ...
                if (!executed && xhr.status === 200) {
                    try {
                        if (xhr.responseText.length > 0) {
                            ...
                            _this.attribute_class = attribute["class"]
                            _this.attribute_llm = attribute["llm"]
