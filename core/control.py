...
import booter
from input_process import xf_ltp
from input_process.ms_tts_sdk import Speech
from utils import wsa_server, tts_voice, song_player
...
from db import qa_service

import pygame
from utils import config_util as cfg
from db import content_db
from datetime import datetime
from input_process import nlp_cemotion

from brain import nlp_rasa
from brain import nlp_gpt
from brain import nlp_yuan
from input_process import yolov8
from brain import nlp_VisualGLM
from brain import nlp_lingju
from brain import nlp_rwkv_api
from brain import nlp_ChatGLM2
from brain import nlp_fastgpt
from brain import nlp_xingchen
from brain import nlp_langchain
...
def send_for_answer(msg,sendto):
        ...
        contentdb.add_content('ben','send',text)
        wsa_server.get_web_instance().add_cmd({"panelReply": {"type":"ben","content":text}})
        if len(textlist) > 1:
            i = 1
            while i < len(textlist):
                  contentdb.add_content('ben','send',textlist[i]['text'])
                  wsa_server.get_web_instance().add_cmd({"panelReply": {"type":"ben","content":textlist[i]['text']}})
                  i+= 1
        return text
                ...
                if len(self.interactive) > 0:
                    ...
                    if index == 1:
                        ...
                        if not config_util.config["interact"]["playSound"]: # 非展板播放
                            ...
                            wsa_server.get_instance().add_cmd(content)
                        #ben eyes
                        ben_eyes = yolov8.new_instance()            
                        if ben_eyes.get_status():#YOLO正在运行
                            person_count, stand_count, sit_count = ben_eyes.get_counts()
                        ...
                        contentdb.add_content('ben','speak',self.a_msg)
                        wsa_server.get_web_instance().add_cmd({"panelReply": {"type":"ben","content":self.a_msg}})
                        if len(textlist) > 1:
                            i = 1
                            while i < len(textlist):
                                contentdb.add_content('ben','speak',textlist[i]['text'])
                                wsa_server.get_web_instance().add_cmd({"panelReply": {"type":"ben","content":textlist[i]['text']}})
                                i+= 1
    ...
    def __say(self, styleType):
        try:
            if len(self.a_msg) < 1:
                ...
                util.printInfo(1, 'benben', '({}) {}'.format(self.__get_mood_voice(), self.a_msg))
                if not config_util.config["interact"]["playSound"]: # 非展板播放
                    content = {'Topic': 'Unreal', 'Data': {'Key': 'text', 'Value': self.a_msg}}
                    wsa_server.get_instance().add_cmd(content)
                MyThread(target=storer.storage_live_interact, args=[Interact('ben', 0, {'user': 'ben', 'msg': self.a_msg})]).start()
