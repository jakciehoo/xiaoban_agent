...
from input_process.ali_nls import ALiNls
from input_process.funasr import FunASR
        ...
        if len(text) > 0:
            if cfg.config['source']['wake_word_enabled']:
                ...
                if cfg.config['source']['wake_word_type'] == 'common':
                    ...
                    if not self.wakeup_matched or self.__ben.speaking:
                        #唤醒词判断
                        ...
                        if wake_up:
                            ...
                            util.log(1, "唤醒成功！")
                            self.__ben.stop_say = True
                            self.on_speaking(text)
                            self.__ben.stop_say = False
            ...
            if percentage > self.__dynamic_threshold and can_listen:
                ...
                if isSpeaking:
                    if time.time() - last_speaking_time > _RELEASE:
                        ...
                        self.__aLiNls.end()
                        util.log(1, "语音处理中...")
                        self.__ben.last_quest_time = time.time()
